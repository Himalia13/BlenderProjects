# BlenderProjects

A collection of Blender project files exploring different visual effects and techniques.

## Black Hole

A procedural black hole with a glowing accretion disk and gravitational lensing effect, made in Blender.

![Black Hole render](BlackHole/0001_comp.png)

[Uncompressed Version](BlackHole/0001.png)

**Tutorial:** [Create Realistic Black Holes in Blender](https://www.youtube.com/watch?v=XWv1Ajc3tfU)

### Thanks

Thanks to [Alaskan FX](https://www.youtube.com/@AlaskanFX) for the tutorial this project is based on.

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.
